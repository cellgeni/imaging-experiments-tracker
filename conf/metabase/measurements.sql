...
al.name as archive_location, td.name as team_directory, experiments_measurement.exported, experiments_measurement.export_status, experiments_measurement.files_path,
concat('http://imaging-tracker.cellgeni.sanger.ac.uk/admin/experiments/measurement/', experiments_measurement.id, '/change/') as url
...
where true [[ and {{researcher}} ]] [[ and {{project}} ]]
[[ and asl.name like concat('%', {{automated_slide_id}}, '%') ]] [[ and pl.name like concat('%', {{automated_plate_id}}, '%') ]] [[ and {{automated_slide_num}} ]]
[[ and sb.name like concat('%', {{ slide_barcode }}, '%') ]]
[[ and experiments_sample.name like concat('%', {{sample_id}}, '%') ]] [[ and {{tissue}} ]] [[ and {{age}} ]] [[ and {{genotype}} ]] [[ and {{background}} ]]
[[ and {{ technology }} ]] [[ and {{image_cycle}} ]] [[ and {{ channel }} ]] [[ and {{ target }} ]] [[ and mes.post_stain like concat('%', {{ post_stain}}, '%') ]]
[[ and experiments_measurement.date >= {{fromdate}} ]] [[ and experiments_measurement.date <= {{untildate}} ]] [[ and {{ measurement_number }}]]
[[ and {{ low_mag_reference }}]] [[ and {{ mag_bin_overlap }}]]
[[ and mes.notes_1 like concat('%', {{notes_1}}, '%') ]] [[ and mes.notes_2 like concat('%', {{notes_2}}, '%') ]]
[[ and {{ meas_id }}]] [[and {{authorized_projects}}]]
