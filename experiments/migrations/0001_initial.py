# Generated by Django 3.0 on 2019-12-09 16:56

from django.db import migrations, models
import django.db.models.deletion
import django.utils.timezone


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='CellGenProject',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('key', models.CharField(max_length=4)),
            ],
        ),
        migrations.CreateModel(
            name='Channel',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=20)),
            ],
        ),
        migrations.CreateModel(
            name='ChannelTarget',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('channel', models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='experiments.Channel')),
            ],
        ),
        migrations.CreateModel(
            name='Pipeline',
            fields=[
                ('name', models.CharField(max_length=20, primary_key=True, serialize=False)),
            ],
        ),
        migrations.CreateModel(
            name='Researcher',
            fields=[
                ('first_name', models.CharField(max_length=30)),
                ('last_name', models.CharField(max_length=30)),
                ('employee_key', models.CharField(max_length=3, primary_key=True, serialize=False)),
            ],
        ),
        migrations.CreateModel(
            name='Sample',
            fields=[
                ('id', models.CharField(max_length=20, primary_key=True, serialize=False)),
                ('species', models.CharField(choices=[], max_length=20)),
                ('age', models.CharField(max_length=20)),
                ('genotype', models.CharField(max_length=20, null=True)),
                ('background', models.CharField(max_length=20, null=True)),
                ('tissue', models.CharField(max_length=20)),
            ],
        ),
        migrations.CreateModel(
            name='Slide',
            fields=[
                ('automated_id', models.CharField(max_length=20)),
                ('barcode_id', models.CharField(max_length=20, primary_key=True, serialize=False)),
            ],
        ),
        migrations.CreateModel(
            name='Target',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=20)),
            ],
        ),
        migrations.CreateModel(
            name='TeamDirectory',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=20)),
            ],
        ),
        migrations.CreateModel(
            name='Section',
            fields=[
                ('number', models.IntegerField(primary_key=True, serialize=False)),
                ('sample', models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='experiments.Sample')),
                ('slide', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='experiments.Slide')),
            ],
        ),
        migrations.CreateModel(
            name='PipelineVersion',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=10)),
                ('pipeline', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='experiments.Pipeline')),
            ],
        ),
        migrations.CreateModel(
            name='Measurement',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('technology', models.CharField(max_length=20)),
                ('automated_plate_id', models.CharField(default=None, max_length=20, null=True)),
                ('automated_slide_num', models.IntegerField(null=True)),
                ('image_cycle', models.IntegerField()),
                ('date', models.DateTimeField(default=django.utils.timezone.now)),
                ('measurement', models.CharField(max_length=20)),
                ('low_mag_reference', models.CharField(max_length=20)),
                ('mag_bin_overlap', models.CharField(max_length=20)),
                ('z_planes', models.CharField(max_length=20)),
                ('microscope', models.CharField(choices=[('phenix', 'Phenix'), ('plateloader', 'Phenix Plateloader')], max_length=20)),
                ('notes_1', models.TextField(max_length=200)),
                ('notes_2', models.TextField(max_length=200)),
                ('channel_target_pairs', models.ManyToManyField(to='experiments.ChannelTarget')),
                ('slide_id', models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='experiments.Slide')),
            ],
        ),
        migrations.CreateModel(
            name='Experiment',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=20)),
                ('team_directory', models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='experiments.TeamDirectory')),
            ],
        ),
        migrations.AddField(
            model_name='channeltarget',
            name='target',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='experiments.Target'),
        ),
        migrations.CreateModel(
            name='Analysis',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('pipelineVersion', models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='experiments.PipelineVersion')),
            ],
        ),
    ]
